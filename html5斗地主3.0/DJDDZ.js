...
DJDDZ.Init = function(canvasID) {
    JFunction.PreLoadData(GMain.URL).done(function() {
        ...
        GMain.BtnPanel = new JControls.Object({ x: 100, y: 380 }, { width: 700, height: 100 }); //用于显示游戏控制按钮
        GMain.PokerPanel0 = new GControls.PokerPanel({ x: 200, y: 20 }, { width: 600, height: 120 }, 0, 0); //用于显示底牌，显示对象存储在GMain.Poker[0]
        GMain.PokerPanel1 = new GControls.PokerPanel({ x: 210, y: 455 }, { width: 570, height: 120 }, 1, 30); //用于显示自己的牌，显示对象存储在GMain.Poker[1]
        GMain.PokerPanel2 = new GControls.PokerPanel({ x: 880, y: 60 }, { width: 100, height: 440 }, 2, 20); //用于显示右边电脑的牌，显示对象存储在GMain.Poker[2]
        GMain.PokerPanel3 = new GControls.PokerPanel({ x: 20, y: 60 }, { width: 100, height: 440 }, 3, 20); //用于显示左边电脑的牌，显示对象存储在GMain.Poker[3]
        GMain.PokerPanel4 = new GControls.PokerPanel({ x: 300, y: 200 }, { width: 400, height: 120 }, 4, 30); //用于显示出的最后一手牌，显示对象存储在GMain.Poker[4]
        var BeginButton = new JControls.Button({ x: 335, y: 0 }, { width: 130, height: 50 }).setBGImage(ResourceData.Images.startbtn);
...
DJDDZ.InitGame = function() {
    ...
    GMain.PokerPanel0.density = 2; //设置扑克牌显示密度
...
DJDDZ.GrabTheLandlord = function() { //抢地主
    ...
    if (GMain.DealerNum == 1) { //自己抢地主
        ...
        var Button1 = new GControls.GrabButton({ x: 110, y: 0 }, { width: 130, height: 50 }, 1).setText("1分").setBGImage(ResourceData.Images.btn);
        var Button2 = new GControls.GrabButton({ x: 260, y: 0 }, { width: 130, height: 50 }, 2).setText("2分").setBGImage(ResourceData.Images.btn);
        var Button3 = new GControls.GrabButton({ x: 410, y: 0 }, { width: 130, height: 50 }, 3).setText("3分").setBGImage(ResourceData.Images.btn);
        var Button4 = new GControls.GrabButton({ x: 560, y: 0 }, { width: 130, height: 50 }).setText("不抢").setBGImage(ResourceData.Images.btn);
...
DJDDZ.GameOver = function() {
    ...
    JMain.JForm.show();
    //alert(success);
    setTimeout(DJDDZ.Go, 2000);
}
DJDDZ.Go = function() {
    window.location.href = "end.html?s=" + success + "&b=" + beishu;
}
var tuoguanf = 0;
DJDDZ.ToPlay = function() { //出牌
    ...
    if (GMain.DealerNum == 1) { //轮到自己出牌
        ...
        if (GMain.LastHandNum == 2 || GMain.LastHandNum == 3) { //不是该轮第一个出牌，可以选择不出牌
            var Button1 = new JControls.Button({ x: 170, y: 0 }, { width: 60, height: 30 }, 1).setBGImage(ResourceData.Images.buchu);
        ...
        var Button2 = new JControls.Button({ x: 370, y: 0 }, { width: 60, height: 30 }).setBGImage(ResourceData.Images.chupai);
        ...
        var Button3 = new JControls.Button({ x: 570, y: 0 }, { width: 60, height: 30 }).setBGImage(ResourceData.Images.tishi);
        ...
        var Button34 = new JControls.Button({ x: 660, y: 25 }, { width: 50, height: 50 }).setText("托管").setBGImage(ResourceData.Images.btn);
        ...
        GMain.BtnPanel.addControlInLast([Button1, Button2, Button3, Button34]);
        GMain.BtnPanel.visible = true;
        ...
        if (tuoguanf == 1) {
            var Button66 = new JControls.Button({ x: 660, y: 25 }, { width: 60, height: 50 }).setText("取消托管").setBGImage(ResourceData.Images.btn);
            Button66.onClick = function() {
                tuoguanf = 0;
            }
            GMain.BtnPanel.addControlInLast([Button66]);
            GMain.BtnPanel.visible = true;
            JMain.JForm.show();
        }
    }
...
DJDDZ.PlayPoker = function(sign) { //出选中的牌
    ...
    if (GMain.Poker[GMain.DealerNum].length == 0) {
        ...
        DJDDZ.GameOver(); //牌出完，游戏结束
    }
}
var GMain = {
    Size: { width: 1000, height: 600 } //屏幕大小
    ,
    URL: "",
    Poker: null,
